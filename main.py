...
async def loghost(ctx, member: discord.Member, *, note=""):
    data = load_data()
    ...
    if data[uid]["loa"] and "(LoA)" not in display_name:
        display_name += " (LoA)"
    response = random.choice(loghost_lines).format(name=display_name, count=data[uid]["monthly"][current_month])
